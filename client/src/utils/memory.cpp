...
namespace
{
	bool equals_ignore_case(const std::string_view str_1, const std::string_view str_2)
	{
		return (str_1.size() == str_2.size()) && std::equal(str_1.begin(), str_1.end(), str_2.begin(),
...
bool c_memory::setup(std::string& error_out)
{
	const auto process_id = this->get_process_id("cs2.exe");
	if (!process_id.has_value())
	{
		error_out = "CS2 is not running.\n\nLaunch the game first, then press Start Radar.";
	...
	const auto hijacked = this->hijack_handle();
	if (hijacked.has_value())
		this->m_handle = hijacked.value();
	else
		this->m_handle = OpenProcess(PROCESS_VM_READ | PROCESS_QUERY_INFORMATION, false, this->m_id);

	if (!this->m_handle)
	{
		error_out = "Found CS2 but could not access it.\n\nTry running AimSync WebRadar as Administrator.";
		LOG_ERROR("failed to open handle to cs2.exe (pid %u)", this->m_id);
		return false;
	}
...
std::optional<uint32_t> c_memory::get_process_id(const std::string_view& process_name)
{
	...
	if (Process32First(snapshot, &process_entry))
	{
		do
		{
			if (equals_ignore_case(process_entry.szExeFile, process_name))
			{
